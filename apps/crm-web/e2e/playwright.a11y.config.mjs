...
// Accessibility (axe-core) harness. Reuses the same self-managed stack as the
// primary e2e config (API on :4100 + built web on :3101) but runs only a11y.spec.mjs.
...
export default defineConfig({
  ...
  reporter: 'list',
  outputDir: path.join(repoRootDir, 'output', 'playwright', 'crm-web-a11y'),
  use: {
