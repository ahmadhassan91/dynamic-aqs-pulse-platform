...
export const APP_LEAD_MARKETING_SOURCES = [
...
export const APP_LEAD_REGION_OPTIONS = [
...
const APP_LEAD_REGION_LOOKUP = new Map(
  APP_LEAD_REGION_OPTIONS.flatMap((option) => [
    [option.value.toUpperCase(), option],
    [option.label.toUpperCase(), option],
  ]),
);

export function findAppLeadRegionOption(value: string | null | undefined) {
  const normalized = value?.trim().toUpperCase();
  if (!normalized) {
    return undefined;
  }

  return APP_LEAD_REGION_LOOKUP.get(normalized);
}
