...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, repeat-submission review APIs/UI under `/leads/forms`, persisted duplicate-review decisions (`confirm existing` / `create new lead`), website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, policy-driven workflow-queue read models, lifecycle controls (`active / parked / closed`), PRD-backed SLA defaults (`24h` initial contact / `72h` discovery scheduling / `5` business-day CIS follow-up), approved US/Canada region validation, marketing-source / lead-rating capture, native website-form address / customer-status metadata preservation, live territory assignment metadata, and a backend-wired lead history feed plus inactive operational views under `/leads/activities` are now locally verified in the production API | Finish downstream alert delivery, deeper duplicate-resolution options (`relink existing`, merge/customer-side resolution), and richer server-side analytics/archived reporting now that queue, history, inactive lifecycle views, and first-pass duplicate review are live | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/contracts/src/lead-options.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs`, `apps/api/test/leads.activity-history.regression.test.mjs` |
...
| Lead module regression suite | `In Progress` | Isolated lead regressions now cover seeded website forms, public form gating, duplicate website capture, repeat-submission review ordering, explicit duplicate-review decisions (`confirm existing`, `create new lead`), website-form archive visibility, archived duplicate-review visibility, inactive-lifecycle duplicate boundaries, native-form address/customer-status metadata preservation, admin site/recipient management, approved region validation, PRD-backed routing/SLA defaults, historical leads missing due dates, discovery scheduling SLA prioritization, CIS follow-up timing, dead/not-interested closure, parked/reopen behavior, dedicated history-feed ordering/search, inactive lifecycle filtering, queue/archive separation, onboarding-readiness blockers, contact-import idempotency, conversion-prep validation, and first-order conversion boundary behavior against a dedicated test database | Expand next into deeper activity/history semantics, relink/merge-style duplicate flows, and lead-to-owner operational reporting | `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs`, `apps/api/test/leads.activity-history.regression.test.mjs`, `apps/api/test/leads.readiness.regression.test.mjs`, `apps/api/test/support/runtime.mjs` |
