...
| Lead module regression suite | `In Progress` | Isolated lead regressions now cover seeded website forms, public form gating, duplicate website capture, native-form address/customer-status metadata preservation, admin site/recipient management, approved region validation, discovery schedule/complete/skip behavior, CIS timestamp transitions, onboarding-readiness blockers, contact-import idempotency, conversion-prep validation, and first-order conversion boundary behavior against a dedicated test database | Expand next into workflow queue prioritization, dead/reopen lifecycle, and additional lead-detail parity edge cases | `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs`, `apps/api/test/leads.readiness.regression.test.mjs`, `apps/api/test/support/runtime.mjs` |
