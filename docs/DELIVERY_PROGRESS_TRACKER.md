...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, website-form read models, workflow-queue read models, approved US/Canada region validation, and marketing-source / lead-rating capture are now locally verified in the production API | Finish prototype-aligned native website-form parity, then add deeper admin-driven workflow configuration and a territory assignment kernel once governance is ready | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/contracts/src/lead-options.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs` |
...
| Web rewire from prototype data layer | `In Progress` | The approved Pulse CRM shell is now enforced at the `/leads` route-layout level, replacing the incorrect standalone UI layer while keeping live lead/pipeline/import/website-forms/workflow-queue/finance/CIS/onboarding wiring under it | Continue broader module-route rewiring on the same prototype shell | `apps/crm-web/src/` |
| Prototype-aligned admin and auth workspace | `In Progress` | Login, protected route gating, admin overview, user management, roles/permissions, and audit activity now run inside the approved prototype shell with real backend wiring | Add runtime smoke verification and extend only when non-placeholder admin scope is approved | `apps/crm-web/src/app/auth/`, `apps/crm-web/src/app/admin/`, `apps/api/src/modules/admin/` |
| Lead module regression suite | `In Progress` | Isolated lead regressions now cover seeded website forms, public form gating, duplicate website capture, admin site/recipient management, approved region validation, discovery schedule/complete/skip behavior, and CIS timestamp transitions against a dedicated test database | Expand next into native-form field parity, workflow queue prioritization, onboarding readiness blockers, and conversion-prep validation | `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs`, `apps/api/test/support/runtime.mjs` |
...
| Wave 1 | Lead Capture & Lead Management | `Ready` | `In Progress` | Backend backbone now covers Pulse-native web intake, hosted website forms, bulk import, service-tech-count-based routing, duplicate-aware website submission tracking, website registry admin APIs, notification-recipient management, workflow queue reads, and the onboarding-readiness boundary, while the production web app now exposes pipeline, import, website forms, workflow queue, finance-linked lead flows, and an onboarding-readiness workspace under the approved prototype shell. |
...
| Lead workflow regression expansion | `Done` | Discovery scheduling/completion/fast-track rules, CIS timestamp transitions, and approved region validation are now covered in the dedicated lead regression suite |
...
| CRM web lead workspace baseline | `In Progress` | The lead workspace now runs inside the approved prototype shell with a shared `/leads` layout, live list/kanban/detail/stage-transition wiring, and no page-level standalone wrapper |
| CRM web workflow queue baseline | `Done` | `/leads/activities` now exists in the restored shell and is wired to the live production lead data / routing policy, providing a real queue instead of a placeholder screen |
| CRM web website forms baseline | `Done` | `/leads/forms` now exists in the restored shell and is wired to live website-form lead observations from the production API instead of a concept-only page |
...
| CRM web auth and admin baseline | `Done` | `/auth/login`, `/admin`, `/admin/users`, `/admin/roles`, and `/admin/activity` now build inside the approved prototype shell with live login, protected-route gating, user management, role matrix, and activity audit flows; concept-only admin tabs are intentionally excluded from the active build |
...
| Lead territory/TM auto-assignment mapping | `Parked` | Routing team selection exists, but named TM/RD assignment should wait for authoritative territory ownership tables and geography rules | When territory reference data is approved |
...
| Shared app-level auth/session provider with refresh handling | `Parked` | The production web routes now use the real backend, but screen-level session persistence still needs to be consolidated into a shared app shell with refresh handling | When the next `crm-web` hardening pass begins |
| Website embed distribution refinements and operational analytics | `Parked` | Core website registry CRUD, hosted form generation, and notification-recipient management are now live; later polish should focus on embed package distribution UX, recent-submission analytics, and recipient-routing refinements once operational usage starts | When real branded-site rollout begins |
...
| Lead-stage alert timing and escalation windows | `Parked` | Stage transitions are now trackable, but approved SLA timers and escalation windows should not be hardcoded before notification policy is signed off | When alerts/notifications configuration is approved |
