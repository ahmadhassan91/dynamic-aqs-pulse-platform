...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, workflow-queue read models, approved US/Canada region validation, marketing-source / lead-rating capture, and native website-form address / customer-status metadata preservation are now locally verified in the production API | Finish deeper admin-driven workflow configuration and the territory assignment kernel once governance is ready | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/contracts/src/lead-options.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs` |
...
| Web rewire from prototype data layer | `In Progress` | The approved Pulse CRM shell is now enforced at the `/leads` route-layout level, and the website-forms experience has been pulled back toward the approved prototype with the correct three-tab management screen, `/admin/lead-capture` and `/settings/lead-capture` aliases, and fuller native-form field capture under the same shell | Continue broader module-route rewiring on the same prototype shell and bring the next lead subviews to the same level of parity | `apps/crm-web/src/` |
...
| Lead module regression suite | `In Progress` | Isolated lead regressions now cover seeded website forms, public form gating, duplicate website capture, native-form address/customer-status metadata preservation, admin site/recipient management, approved region validation, discovery schedule/complete/skip behavior, and CIS timestamp transitions against a dedicated test database | Expand next into workflow queue prioritization, onboarding readiness blockers, conversion-prep validation, and additional native-form submission edge cases | `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs`, `apps/api/test/support/runtime.mjs` |
...
| CRM web website forms baseline | `Done` | `/leads/forms`, `/admin/lead-capture`, and `/settings/lead-capture` now resolve to the same restored prototype-style management screen, while the public hosted form captures the fuller replacement-HubSpot intake payload against the live production API |
