...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, website-form read models, and workflow-queue read models are now locally verified in the production API | Add deeper admin-driven workflow configuration and then a territory assignment kernel once governance is ready | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs` |
...
| Lead module regression suite | `In Progress` | The first isolated module-level regression suite now covers seeded website forms, public form gating, duplicate website capture behavior, and admin site/recipient management against a dedicated test database | Expand coverage into workflow queue, discovery, CIS, and onboarding readiness | `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/support/runtime.mjs` |
