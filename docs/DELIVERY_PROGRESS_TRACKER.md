...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, policy-driven workflow-queue read models, lifecycle controls (`active / parked / closed`), PRD-backed SLA defaults (`24h` initial contact / `72h` discovery scheduling / `5` business-day CIS follow-up), approved US/Canada region validation, marketing-source / lead-rating capture, and native website-form address / customer-status metadata preservation are now locally verified in the production API | Finish territory assignment, duplicate-resolution UX, and downstream alert delivery once governance and provider dependencies are ready | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/contracts/src/lead-options.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs` |
