...
| Lead intake and routing backbone | `In Progress` | Service-tech-count-based lead intake, routing policy, public website capture, duplicate-aware website submission tracking, repeat-submission review APIs/UI under `/leads/forms`, website registry admin APIs, notification-recipient APIs, internal lead APIs, stage history, policy-driven workflow-queue read models, lifecycle controls (`active / parked / closed`), PRD-backed SLA defaults (`24h` initial contact / `72h` discovery scheduling / `5` business-day CIS follow-up), approved US/Canada region validation, marketing-source / lead-rating capture, native website-form address / customer-status metadata preservation, and live territory assignment metadata are now locally verified in the production API | Finish downstream alert delivery, duplicate-resolution decisions beyond auto-attach review, and the deeper named-owner override flows now that the territory kernel is live | `apps/api/src/modules/leads/`, `packages/contracts/src/leads.ts`, `packages/contracts/src/lead-options.ts`, `packages/db/prisma/schema.prisma`, `apps/api/test/leads.website-forms.regression.test.mjs`, `apps/api/test/leads.workflow.regression.test.mjs` |
