...
| --- | ---: | --- |
| Leads | 79% | Intake, website forms, duplicate review, routing, workflow queues, OCR preview, readiness, operational alerts, and mobile workflow-queue visibility are regression/UAT-covered. Mobile committed scan-to-lead remains a gap. |
...
- Mobile native simulator depth now has a TM production-API pass for home, workflow-queue leads, account context, route draft restore, and CRM checkout save. Stored-session validation, proactive refresh, checked-in route-draft durability, approved production API host controls, live-API partial/unavailable dashboard status, and manual-sync next-action guidance now have implementation and unit coverage. Remaining mobile blockers are Android QA, true background sync/conflict merge, push/deep links, and offline binary media.
