...
- Added the dependency-free Field Operating Day seed and regression: source lead to account lineage, scheduled training, active ROSE consignment site/audit with signed agreement/BLUE/ROSE records, four contextual mobile notes, office review writeback to lead/account/training/consignment read models, and manual PO follow-up creation for a ROSE variance while Acumatica remains parked.
- Mobile Voice Notes now supports simple CRM context selection for General, Lead, Account, Training Session, Consignment Site, and Route Visit notes instead of account-only capture.
...
| Field Operating Day UAT seed | `pnpm --filter @pulse/api test:uat-readiness` passed, covering seeded personas/catalogs plus lead/account/training/consignment contextual field notes, office review writeback, ROSE variance, and manual PO follow-up boundary. |
| Contextual mobile Voice Notes | `pnpm --filter @pulse/mobile test` passed, 34/34; `pnpm --filter @pulse/mobile typecheck` passed. Mobile tests now cover lead, account, training session, consignment site, route visit, and general voice-note request payloads. |
...
| --- | ---: | --- |
| Leads | 81% | Intake, website forms, duplicate review, routing, workflow queues, OCR preview, readiness, operational alerts, mobile workflow-queue visibility, seeded source-lead lineage, contextual lead voice-note capture, and approved mobile field notes in lead activity are regression/UAT-covered. Mobile committed scan-to-lead remains a gap. |
| Accounts / Customers | 82% | Live customer list/detail, profile edit, contacts, locations, lifecycle, territory ownership, source-lead lineage, dealer portal access, payment-method boundary, Account Readiness, day-one handoff guidance, Today&apos;s Account Focus, stable tab deep links, CRM-owned Activity & Document Review, contextual account voice-note capture, and approved mobile field notes in account activity are covered. Parent/child hierarchy, merge/import governance, ERP orders/invoices/shipments/pricing, and external document truth remain parked. |
...
| Training | 81% | Training catalog, programs, sessions, check-in/out, proof metadata/upload backend, certification, exceptions, reporting, seeded field-day session, contextual training voice-note capture, mobile follow-up/proof-failure truthfulness, and reviewed mobile voice-note follow-up task creation are covered. Named attendee/technician depth remains limited. |
| Consignment | 74% | Site master, document register, seeded active ROSE site/audit, signed agreement/BLUE/ROSE records, evidence upload, variance, manual PO follow-up boundary, queue, Acumatica boundary, contextual consignment voice-note capture, and reviewed mobile voice-note field work items are covered. ERP warehouse/inventory/PO truth remains parked. |
...
Overall dependency-free readiness: approximately 83%.
