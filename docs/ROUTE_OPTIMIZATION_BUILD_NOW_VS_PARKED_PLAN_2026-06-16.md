...
| `RTE-P5` | Dwell + end-of-day ETA | Per-stop editable dwell time (default 30 min, 1–60) + an adjustable route start time; a straight-line "miles → rough drive minutes" estimate to compute a cumulative per-stop arrival + end-of-day ETA, clearly labelled as an estimate. Drive time = (straight-line miles × a ~1.25–1.3× **circuity factor**, approximating road distance from as-the-crow-flies) ÷ a flat avg-mph constant. Entirely **provider-free**: no Google/Mapbox, no API key, no billing — only the haversine distance already computed in RTE-P3. The circuity factor + speed constant are the honest, calibrated stand-in until the traffic-accurate ETA (RTE-K2) lands | `apps/mobile/app/(tabs)/route.tsx`, `apps/mobile/src/lib/route-planning.ts` |
