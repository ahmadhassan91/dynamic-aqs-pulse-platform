...
| ID | Requirement | Acceptance Criteria | Priority | Build Status | SRC |
|---|---|---|---|---|---|
| FR-MOB-061 | Pulse Mobile shall provide a Map-My-Customer-parity route builder: the user selects multiple stops from the colour-coded map, sets a per-stop dwell time (1–60 min), and the system computes an optimized start→finish order with an end-of-day ETA, supporting multiple/saved routes; colour-coding is preserved on the route-building screen and turn-by-turn is handed off externally (FR-MOB-034). | Multi-select stops → optimized ordered route with per-stop dwell + ETA; routes can be saved; pins keep their colour through route building | P1 | Built (buildable-now slices RTE-P1–P6): multi-select route builder with add/remove/reorder, Optimize via client nearest-neighbour that consumes the existing server route plan (459d069, f843a46), per-stop dwell + straight-line end-of-day ETA (b41f062), saved/named routes (local-first, 6b00087), and colour-coding preserved through routing (a3b85e1). PARKED behind the maps-provider decision (OQ-MOB-08): real road-distance/traffic-aware ETA + a true distance-optimized TSP (RTE-K2), and server-side route sync / manager share-export (RTE-K4). Build plan: `docs/ROUTE_OPTIMIZATION_BUILD_NOW_VS_PARKED_PLAN_2026-06-16.md` | SRC-MOB-002 (Don, Session 4: 1–60 min dwell, "at your hotel by seven o'clock", multiple routes) |
