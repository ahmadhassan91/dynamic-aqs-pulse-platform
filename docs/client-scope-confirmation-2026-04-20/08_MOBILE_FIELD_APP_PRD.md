...
| Version | 2.2 |
| Date | 2026-06-16 |
| Status | Meeting-research depth pass — enriched three under-detailed requirements with verbatim meeting evidence: in-app on-behalf ordering (CG called it "critical"; Acumatica-trigger flow), route optimization (the full Map My Customer route-builder behaviour Don described), and map colour-coding (CG's "don't change colors" + the pending Currie colour-list deliverable). Builds on the 2026-06-14 build-status reconciliation pass |
...
| FR-MOB-034 | Route navigation shall offer a choice of Google Maps, Apple Maps, or Waze for turn-by-turn handoff. In-app turn-by-turn is explicitly NOT built; navigation is delegated to the external app for the selected stop — the Map My Customer model Don and the BA confirmed in Session 4. | Navigation handoff button opens the chosen provider for the selected stop | P1 | Not-built (navigation handoff not wired; parked pending the maps-provider decision — OQ-MOB-08) | SRC-MOB-002 |
...
| ID | Requirement | Acceptance Criteria | Priority | Build Status | SRC |
|---|---|---|---|---|---|
| FR-MOB-061 | Pulse Mobile shall provide a Map-My-Customer-parity route builder: the user selects multiple stops from the colour-coded map, sets a per-stop dwell time (1–60 min), and the system computes an optimized start→finish order with an end-of-day ETA, supporting multiple/saved routes; colour-coding is preserved on the route-building screen and turn-by-turn is handed off externally (FR-MOB-034). | Multi-select stops → optimized ordered route with per-stop dwell + ETA; routes can be saved; pins keep their colour through route building | P1 | Not-built — the optimization (TSP) engine is parked behind the maps-provider decision (OQ-MOB-08); interim is oldest-last-touch ordering (FR-MOB-027). Prototype demoed multi-select + optimize against a placeholder provider. Build plan (buildable-now nav handoff + builder consuming the existing server route plan vs provider-parked TSP/traffic ETA): `docs/ROUTE_OPTIMIZATION_BUILD_NOW_VS_PARKED_PLAN_2026-06-16.md` | SRC-MOB-002 (Don, Session 4: 1–60 min dwell, "at your hotel by seven o'clock", multiple routes) |
