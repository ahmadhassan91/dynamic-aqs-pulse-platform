...
| Version | 2.5 |
| Date | 2026-06-19 |
| Status | Order-on-behalf delivery (2026-06-19): the buildable-now CRM `OrderDraft` slice for FR-MOB-047 is SHIPPED (backend ORD-P1–P3 + mobile capture UI ORD-P4) — see the §BUILD note below; the Acumatica order placement remains parked. Builds on v2.4's scope-accuracy corrections and v2.3's route-optimization build reconciliation |
...
**Parked (Acumatica-gated, ORD-K-series):** live `SalesOrder`/`Customer` create, first-order-creates-account, authoritative price/tax/shipping, inbound order reflection. Next CRM-side slices: ORD-P5 (first-order signal into `convertLeadOnFirstOrder`) and ORD-P6 (web back-office triage queue). Full split: `docs/ORDER_ON_BEHALF_BUILD_NOW_VS_ACUMATICA_PARKED_PLAN_2026-06-16.md`.
