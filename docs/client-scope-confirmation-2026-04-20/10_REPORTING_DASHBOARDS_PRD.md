...
| FR-RPT-003 | Configurable "active account" threshold | Admin can configure the window (e.g. ordered in last N months) that defines an "active account" for all dashboard KPI counts. C G: "Active accounts — we should be able to change that parameter." (SRC-RPT-001) | P1 | Not-built — not a quick win: no admin-settings store exists (needs a settings model + CRUD + admin UI) and "active account = ordered in last N months" depends on order history (Pulse OrderDraft is partial; full order data is Acumatica-parked) | SRC-RPT-001 |
