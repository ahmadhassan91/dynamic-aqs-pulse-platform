...
| FR-RPT-022 | Partial | **Built** | stage counts + per-stage aging (avg/max/stale) — commit 00773d1/3dc4034 |
...
| FR-RPT-008 | Not-built | **Built** | CSV + Excel (.xls SpreadsheetML) + PDF (jsPDF) download on report runs — commit 9ba66fa; plus dashboard-tile export on all three role dashboards (Lead SLA-at-risk, Training overdue, Executive exceptions — 2e68274/6f79a4d/5272f73) via a reusable `downloadTable*` core |
...
| FR-RPT-003 | Configurable "active account" threshold | Admin can configure the window (e.g. ordered in last N months) that defines an "active account" for all dashboard KPI counts. C G: "Active accounts — we should be able to change that parameter." (SRC-RPT-001) | P1 | **Built** (FeatureFlag store `reporting.thresholds` + admin UI + audited — `ed35ac1`/`1eb1fba`; the active-account *count* consuming it stays Acumatica-gated) | SRC-RPT-001 |
...
| FR-RPT-015 | Lost account tracking | Report showing accounts that were active and have not ordered within the configured active-account window. Michelle: "We also like to see lost accounts." (SRC-RPT-001) | P0 | **Partial** (engagement-based staleness shipped via the neglected-accounts report — `e98f96d`/`0cfc65f`; the order-based "lost" definition stays Acumatica-gated) | SRC-RPT-001 |
...
| FR-RPT-022 | Lead stage bottleneck view — count per stage, aging | Shows how many leads sit in each pipeline stage and how long they have been there. SRC-RPT-004: "Lead stage bottlenecks and next gate." | P0 | **Built** (counts + per-stage aging avg/max/stale via getWorkflowStageAnchorAt, server-side aggregated — `00773d1`/`3dc4034`) | SRC-RPT-001, SRC-RPT-004 |
...
| FR-RPT-027 | SLA risk and ownership handoff alerts | Highlights leads that are at risk of missing their stage-transition SLA and who currently owns them. SRC-RPT-004: "March 13 strongly reinforced handoff alerts." | P0 | **Built** (SLA-at-risk list: overdue uncontacted leads with owner + days-overdue, most-overdue-first — `68ea5ec`) | SRC-RPT-004 |
...
| FR-RPT-039 | 90-day account engagement staleness alert | TM dashboard flags accounts where last contact (email, phone, visit, training) was more than 90 days ago. | P0 | **Built** (now backed by a real signal — `Account.lastEngagementAt` is maintained on training/visit, field voice notes, ROSE audits — `a78df8e`; plus a record-scoped neglected-accounts report + card — `e98f96d`/`0cfc65f`) | SRC-RPT-009 |
...
| FR-RPT-067 | Chart type selection in builder: bar, line, area | User can pick chart type for any built report. Ahmad: "Different chart options by area, line, and bar." (SRC-RPT-001) | P1 | **Partial** (the shared ReportChart toggle exists + is used on dashboards; wiring it into the report-builder output is pending) | SRC-RPT-001 |
...
CRM-native reporting build-out shipped on branch `codex/entra-calendar-governance` (full api regression 370/370; contracts/api/web tsc + eslint clean):
- **Chart foundation** — `@mantine/charts` + shared `ReportChart` (bar/line/area toggle, FR-RPT-006) applied to Lead/Training/Executive dashboards — `f48fa32` `f8a80df` `f523acf`.
- **Lead-health detail** — per-stage aging (FR-RPT-022) + SLA-at-risk/handoff list (FR-RPT-027), server-side aggregated — `68ea5ec` `00773d1` `3dc4034`.
- **Configurable thresholds** (FR-RPT-003) — FeatureFlag store + admin UI + audited — `ed35ac1` `1eb1fba`.
