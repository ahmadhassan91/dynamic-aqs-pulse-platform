...
- **GAP-N2 — New in-app triggers from existing Pulse data:** CIS-received ✅ **DONE** (`d46da93` — fires on `cisSubmittedAt` to assignee + RD + routed recipients, deep-linked, idempotent). Remaining: billing/contact-changed (account update hook); visit-inactivity (no training/visit in 6 months). *(M each)*
