...
| Customer search/list | Customer PRD | Partial | searchable list by display/legal/account number is live in `CustomerList.tsx` and `apps/api/src/modules/accounts/service.ts`, and lifecycle filtering is now supported | Add pagination, configurable columns, and bulk actions |
...
| Customer lifecycle state machine (`Active / At Risk / Inactive / Churned`) | Customer PRD | Implemented | account lifecycle enum, governed transition API, lifecycle list filters, customer detail actions, and regressions are now live in `apps/api/src/modules/accounts/service.ts`, `apps/crm-web/src/components/customers/CustomerOverview.tsx`, and `apps/api/test/accounts.regression.test.mjs` | Add lifecycle automation once order/read-model signals are available |
| At-risk flag and no-order aging | Customer PRD | Partial | manual lifecycle states and lifecycle audit are now live, but automatic no-order aging is still parked | Build once ERP/order read models are present or an interim order signal exists |
...
1. Add order-driven lifecycle automation and alerts
