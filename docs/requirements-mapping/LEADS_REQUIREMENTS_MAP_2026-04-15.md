...
| --- | --- | --- | --- | --- |
| Direct branded website intake replacing HubSpot | Lead PRD + March session 8 | Implemented | `apps/api/src/modules/leads/service.ts`, `apps/api/src/modules/leads/website-config.ts`, `apps/crm-web/src/components/leads/PublicWebsiteLeadCaptureForm.tsx`, `apps/api/test/leads.website-forms.regression.test.mjs` | Keep this as the canonical intake path |
| Per-website native form configuration with hosted preview and embed distribution | Discovery sessions 2/4/9 + prototype lead-capture workspace | Partial | `apps/api/src/modules/leads/website-config.ts`, `apps/crm-web/src/components/leads/LeadWebsiteFormsWorkspace.tsx`, `docs/WEBSITE_NATIVE_FORMS_CONNECTION_MODEL_2026-04-15.md` | Add verified-origin / connection-status hardening so website rollout is governed, not trust-only |
| Public website capture enforces site-configured inquiry/referral options | Discovery-backed native forms replacement + production hardening | Implemented | `apps/api/src/modules/leads/service.ts`, `apps/api/src/modules/leads/shared.ts`, `apps/api/test/leads.website-forms.regression.test.mjs` | Extend later into field-level schema-driven requiredness if admin builder scope is approved |
...
| CSV/XLSX import with header mapping | Lead PRD + approved sheets | Implemented | `apps/api/src/modules/leads/file-ingest.ts`, `/leads/import`, `apps/api/test/leads.import.regression.test.mjs`, `docs/LEAD_IMPORT_GOVERNANCE_IMPLEMENTATION_2026-04-15.md` | Keep this as the canonical import path |
...
| Immediate BD-team alerting on website lead capture | March session 8 + lead PRD supplement | Partial | recipient configuration exists and is admin-managed through `WebsiteLeadNotificationRecipient`, but outbound dispatch path is still not fully evidenced | Verify/build real outbound alert dispatch when email provider is approved |
...
| Import duplicate preview with merge/skip/create decisions | Lead PRD import requirements | Implemented | persisted import review runs now support `create_new` / `use_existing` / `skip`, within-file duplicate detection, durable run fetch, and governed commit in `apps/api/src/modules/leads/service.ts`, `/leads/import`, and `apps/api/test/leads.import.regression.test.mjs` | Add merge/update semantics only after governance rules are formally signed off |
| Dedupe across leads and existing customers | Lead PRD | Partial | website dedupe and persisted import-run dedupe now exist, including within-file duplicate detection and account-aware review | Expand into a shared dedupe kernel for manual intake and deeper customer-match resolution |
...
1. Add verified origin / connection-status hardening for branded website rollout
2. Resolve routing-basis decision and lock seed/default policy
...
5. Add merge/update semantics only after cross-entity governance is approved
