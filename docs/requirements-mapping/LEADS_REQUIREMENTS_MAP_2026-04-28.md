...
| Non-linear discovery, CIS, and onboarding sequence | `18 feb 2026 Discovery session 2.md` explicitly says steps do not always happen in the preferred order | Partial | readiness and workflow already allow some flexible progression in `apps/api/src/modules/leads/readiness.ts` and `service.ts` | Remove remaining rigid assumptions from analytics, gating, and operator UX |
